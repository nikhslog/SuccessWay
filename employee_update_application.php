<?php
// employee_update_application.php
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $status_stmt = $conn->prepare("SELECT a.status, a.university_name, a.program, s.email, s.full_name FROM applications a 
    ...
    $university = $status_data['university_name'];
    $program = $status_data['program'];
    $status_stmt->close();
    
    // Update application status
    $stmt = $conn->prepare("UPDATE applications SET status = ?, notes = ? WHERE application_id = ?");
    ...
    if ($stmt->execute()) {
        ...
        if ($current_status != $new_status) {
            ...
            $log_stmt->bind_param("issi", $application_id, $current_status, $new_status, $employee_id);
            $log_stmt->execute();
            $log_stmt->close();
...
function sendStatusUpdateEmail($email, $student_name, $application_id, $old_status, $new_status, $university, $program) {
    // Email subject
    $subject = "SuccessWay Application Status Update - #" . $application_id;
    
    // Email headers
    $headers = "MIME-Version: 1.0" . "\r\n";
    $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
    $headers .= "From: SuccessWay <noreply@successwaymali.com>" . "\r\n";
    ...
    <head>
        <style>
            ...
            .old-status { background-color: #ffeaa7; color: #d69e2e; }
            .new-status { background-color: #c6f6d5; color: #38a169; }
        </style>
    </head>
    <body>
        <div class="container">
            <div class="header">
                <h2>SuccessWay Application Status Update</h2>
            </div>
            <div class="content">
                <p>Dear ' . htmlspecialchars($student_name) . ',</p>
                
                <p>We are writing to inform you that the status of your application to <strong>' . htmlspecialchars($university) . '</strong> 
                for the program <strong>' . htmlspecialchars($program) . '</strong> has been updated.</p>
                
                <p>Application ID: <strong>#' . $application_id . '</strong></p>
                
                <p>Status changed from: <span class="status old-status">' . htmlspecialchars($old_status) . '</span>
                to <span class="status new-status">' . htmlspecialchars($new_status) . '</span></p>';
    
    // Add specific messages based on the new status
    if ($new_status == 'Under Review') {
        $body .= '<p>Your application is now being reviewed by our team. We will process it as quickly as possible.</p>';
    } elseif ($new_status == 'Sent to University') {
        $body .= '<p>Great news! Your application has been sent to the university. We will inform you once we receive any feedback.</p>';
    } elseif ($new_status == 'Accepted') {
        $body .= '<p>Congratulations! Your application has been accepted. Our team will contact you soon to discuss the next steps.</p>';
    } elseif ($new_status == 'Rejected') {
        $body .= '<p>Unfortunately, your application was not successful this time. Please contact our office to discuss alternative options or improvements for future applications.</p>';
    }
    
    $body .= '
                <p>You can log in to your SuccessWay student portal to check the details and track your application status.</p>
                
                <p>If you have any questions, please don\'t hesitate to contact us.</p>
                
                <p>Best regards,<br>
                The SuccessWay Team</p>
            </div>
            <div class="footer">
                <p>This is an automated message, please do not reply to this email.</p>
                <p>&copy; ' . date('Y') . ' SuccessWay. All rights reserved.</p>
            </div>
        </div>
    </body>
    </html>
